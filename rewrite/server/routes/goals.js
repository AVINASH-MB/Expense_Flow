...
const row = (r) => ({
  ...
  deadline: r.deadline
    ? r.deadline instanceof Date ? r.deadline.toISOString().slice(0, 10) : r.deadline
    : undefined,
});
...
router.put("/:id", async (req, res, next) => {
  try {
    ...
    const [r] = await pool.query(
      `UPDATE goals SET ${sets.join(",")} WHERE id=? AND user_id=?`, vals,
    );
...
router.delete("/:id", async (req, res, next) => {
  try {
    const [r] = await pool.query(
      "DELETE FROM goals WHERE id=? AND user_id=?", [req.params.id, req.user.id],
    );
    if (!r.affectedRows) return res.status(404).json({ error: "Not found" });
    res.json({ ok: true });
  } catch (e) { next(e); }
